...
var pumpRunValue = webMI.query['PumpRun'];
var pumpName = webMI.query['name'];

webMI.data.subscribe('AGENT.OBJECTS.IVE50.Mud.Pump.StrokePump1', function () {

  if (pumpRunValue && pumpName) {

    webMI.data.read(pumpRunValue, function (ev) {
      console.log('== PUMP RUN ==', ev.value, 'PUMP NAME === ', pumpName);

      webMI.data.read(number, function (e) {
        pumpFlowRendering(e, 'null');
      });
    });
  }
});
...
webMI.data.subscribe(number, function (e) {
  pumpFlowRendering(e, 'default');
});

function pumpFlowRendering(event, type) {
  ...
  webMI.data.read(webMI.query['MaxValue'], function (f) {
    ...
    var min = 0;
    //console.log('8=====D',max)
    var restrict1 = max / 100 * 61; //* 48.8
    var restrict3 = max / 100 * 81; //* 64.8
    var value = e.value;
    ...
    if (type == 'default') {
      ...
      var bluestr = "rotate(-180deg)"
      blue_circle.style.WebkitTransform = bluestr
      yellow_circle.style.WebkitTransform = bluestr
      red_circle.style.WebkitTransform = bluestr
    }
  });
}
